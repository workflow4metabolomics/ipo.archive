...
ipo4retgroup = function(xset, directory, parametersOutput, listArguments, samplebyclass=4) {
    ...
    # export
    resultRetcorGroup_best_settings_parameters = resultRetcorGroup$best_settings$parameters[!(names(resultRetcorGroup$best_settings$parameters) %in% c("nSlaves"))]
    write.table(resultRetcorGroup_best_settings_parameters, file=parametersOutput, sep="\t",row.names=FALSE) #can be read by user
